...
function proposalForUser(proposal, user) {
  ...
  return ["manager", "director", "admin"].includes(user.role);
}
...
export async function handleApi(req, res) {
  ...
  try {
    ...
    if (method === "PATCH" && assignMatch) {
      requireRole(current, ["manager", "admin"]);
      const body = await readBody(req);
    ...
    if (method === "POST" && sendFinalMatch) {
      requireRole(current, ["cm"]);
      const proposal = db.proposals.find((item) => item.id === sendFinalMatch[1] && item.assignedCmEmail === current.email);
      if (!proposal) throw Object.assign(new Error("Proposal not found"), { status: 404 });
      let sent = 0;
      for (const sku of proposal.skus) {
    ...
    if (method === "PATCH" && cmDecisionMatch) {
      ...
      if (!found || found.proposal.assignedCmEmail !== current.email) throw Object.assign(new Error("SKU not found"), { status: 404 });
    ...
    if (method === "GET" && url.pathname === "/api/products") {
      return sendJson(res, 200, { products: db.products.filter((product) => current.role !== "supplier" || product.supplierId === current.supplierId) });
    }
    ...
    if (method === "GET" && url.pathname === "/api/notifications") {
      const notifications = db.notifications.filter((notification) =>
        ["manager", "admin"].includes(current.role) || notification.recipientEmail === current.email,
      );
...
async function createSession(res, db, user) {
  ...
  await saveDb(db);
  return sendJson(res, 200, { user: publicUser(user, db) }, { "set-cookie": `sid=${sid}; HttpOnly; SameSite=Lax; Path=/; Max-Age=43200` });
}
